...
def test_vectorstore_documents_flow():
    ...
    vectorstore_response = client.post(
        f"/api/v1/users/{user_id}/vectorstores/", json=vs_payload
    )
    ...
    response = client.post(
        f"/api/v1/vectorstores/{vectorstore_id}/documents/", json=add_texts_payload
    )
    assert response.status_code == 200, response.text
    ...
    response = client.post(
        f"/api/v1/vectorstores/{vectorstore_id}/search/", json=search_payload
    )
    assert response.status_code == 200, response.text
