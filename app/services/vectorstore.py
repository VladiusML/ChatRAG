...
class PostgresVectorStoreService:
    def __init__(
        ...
        """
        self.connection_config = connection_config or settings.DATABASE_CONNECTION_CONFIG
    ...
    def get_vectorstore(self, db: Session, vectorstore_id: int) -> Optional[VectorStore]:
        """
        ...
        """
        return db.query(VectorStore).filter(VectorStore.vectorstore_id == vectorstore_id).first()
    
    def get_vectorstores_for_user(self, db: Session, user_id: int) -> List[Dict[str, Any]]:
        """
        ...
        """
        stores = []
        vectorstores = db.query(VectorStore).filter(VectorStore.user_id == user_id).all()
        
        for vs in vectorstores:
            document_count = db.query(DBDocument).filter(DBDocument.vectorstore_id == vs.vectorstore_id).count()
            stores.append({
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    LIMIT %s
                    """,
                    (query_embedding, vectorstore_id, 
                    query_embedding, k)
                )
